...
__export(index_exports, {
  DEFAULT_REACT_NATIVE_COMPATIBILITY: () => DEFAULT_REACT_NATIVE_COMPATIBILITY,
  DEPRECATED_NATIVE_PACKAGES: () => DEPRECATED_NATIVE_PACKAGES,
  REACT_NATIVE_COMPATIBILITIES: () => REACT_NATIVE_COMPATIBILITIES,
  createCustomResolver: () => createCustomResolver,
  getTossReactNativePackages: () => getTossReactNativePackages,
  reactNativeCompatibilityPlugin: () => reactNativeCompatibilityPlugin,
  reactNativeReverseCompatibilityPlugin: () => reactNativeReverseCompatibilityPlugin
});
...
var REACT_NATIVE_COMPATIBILITIES = {
  ...
  "0.72.6": {
    ...
    getReactNativeBasePath: () => {
      return import_path.default.dirname(require.resolve("react-native-0.72.6"));
    },
    getHermesBinaryPath: () => {
      const rnBasePath = import_path.default.dirname(require.resolve("react-native-0.72.6"));
      const hermesEngine = import_path.default.join(rnBasePath, "sdks", "hermesc", getHermesOSBin(), getHermesOSExe());
      return hermesEngine;
    }
  }
};
...
var builtinModules = new Set(import_module.default.builtinModules);
...
function createCustomResolver(rootPath, options) {
  ...
  function createResolverImpl(context, platform, rootPath2) {
    ...
    const resolver = import_enhanced_resolve.default.create.sync({
      ...
      mainFiles: ["index"],
      modules: ["node_modules", import_path2.default.join(rootPath2, "src")]
    });
    function resolve(context2, request) {
      ...
      if (matchedAlias != null) {
        const resolveResult = resolver(
          {},
          matchedAlias.resolveDir ?? import_path2.default.dirname(context2.originModulePath),
      ...
      try {
        const resolveResult = resolver({}, import_path2.default.dirname(context2.originModulePath), request);
        return {
          type: "sourceFile",
          filePath: resolveResult
        };
...
function resolveAliasConfig(alias) {
  return alias.map((alias2) => {
    ...
    const isResolvedPath = import_path2.default.isAbsolute(alias2.to);
    return {
      ...alias2,
...
function replaceModulePath(path5, from, to) {
  const subpath = path5.slice(from.length);
  return `${to}${subpath}`;
}
...
function reactNativeCompatibilityPlugin(options) {
  ...
  if (isReactNativeVersionOverridden) {
    compatibilityReactNativePath = import_path3.default.dirname(require.resolve(`${compatibilityReactNativePackage}/package.json`));
  }
  const dependencyAliasResolveCaches = /* @__PURE__ */ new Map();
  const dependencyAliasConfigs = Object.entries(compatibilityAliases).map(([from, to]) => ({
    from,
    to: async (context) => {
  ...
  const metroDependencyAliasConfig = Object.entries(compatibilityAliases).map(([from, to]) => ({
    from,
    to,
    resolveDir: __dirname
  }));
  return {
    ...
    config: {
      ...
      resolver: {
        alias: [initializeCoreAliasConfig, ...dependencyAliasConfigs].filter(import_es_toolkit.isNotNil)
      },
      esbuild: {
      ...
      metro: {
        resolver: {
          resolveRequest: target === "0.72.6" ? createCustomResolver(options.rootDir, {
            alias: metroDependencyAliasConfig
          }) : void 0
        }
      }
    }
...
function extractAppBridgePayload(input) {
  ...
  if (!("params" in input) && !("callbacks" in input)) {
    return null;
  }
  const payload = input;
  return {
...
function callMiniAppMethod(moduleRef, methodName, params, callbacks) {
  ...
  if (hasCallbacks || isAppBridgePayload || APP_BRIDGE_AD_METHODS.includes(methodName)) {
    return directMethod({
...
function createTossCoreModuleCompat(reactNative, miniAppModule) {
  const tossCoreModule = getNativeModule(reactNative, ["TossCoreModule"]);
  if (isRecord(tossCoreModule)) {
    return tossCoreModule;
  }
  return {
    moduleName: "TossCoreModule",
    eventLog: (params) => Promise.resolve(callMiniAppMethod(miniAppModule, "eventLog", params))
  };
}
function createBrickModuleCompat(globalRef) {
  ...
  const moduleFactories = {
    ...
    TossCoreModule: () => createTossCoreModuleCompat(getReactNativeRuntime(), getMiniAppModule()),
...
function createReverseBrickModuleCompatContent({ isHost }) {
  return [
    ...
    `const APP_BRIDGE_AD_METHODS = ${JSON.stringify([...APP_BRIDGE_AD_METHODS])};`,
    isRecord.toString(),
...
function ensureReverseCompatFiles({ rootDir, isHost }) {
  const graniteDirectoryPath = import_path4.default.join(rootDir, GRANITE_DIRECTORY_NAME);
  const brickModuleCompatPath = import_path4.default.join(graniteDirectoryPath, BRICK_MODULE_COMPAT_FILE_NAME);
  try {
    const reverseBrickModuleCompatContent = createReverseBrickModuleCompatContent({ isHost });
    import_fs.default.mkdirSync(graniteDirectoryPath, { recursive: true });
    import_fs.default.writeFileSync(brickModuleCompatPath, reverseBrickModuleCompatContent, "utf8");
  } catch (error) {
