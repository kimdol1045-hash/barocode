...
function replaceModulePath(path6, from, to) {
  const subpath = path6.slice(from.length);
  return `${to}${subpath}`;
}
...
function reactNativeCompatibilityPlugin(options) {
  ...
  if (isReactNativeVersionOverridden) {
    compatibilityReactNativePath = path4.dirname(__require.resolve(`${compatibilityReactNativePackage}/package.json`));
  }
...
function ensureReverseCompatFiles({ rootDir, isHost }) {
  const graniteDirectoryPath = path5.join(rootDir, GRANITE_DIRECTORY_NAME);
  const brickModuleCompatPath = path5.join(graniteDirectoryPath, BRICK_MODULE_COMPAT_FILE_NAME);
  try {
    ...
    fs.writeFileSync(brickModuleCompatPath, reverseBrickModuleCompatContent, "utf8");
  } catch (error) {
